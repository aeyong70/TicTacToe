...
public static class TicTacToeAI
{
    ...
    public static (int row, int col)? GetBestMove(Constants.PlayerType[,] board)
    {
        ...
        for (int row = 0; row < board.GetLength(0); row++)
        {
            for (int col = 0; col < board.GetLength(1); col++)
            {
                ...
                {
                    board[row, col] = Constants.PlayerType.PlayerB; // AI의 턴
    ...
    {
        // 게임 결과 확인
        if (CheckGameWin(Constants.PlayerType.PlayerA, board)) return -10 + depth;
        if (CheckGameWin(Constants.PlayerType.PlayerB, board)) return 10 - depth;
        ...
        else
        {
            ...
            for (int row = 0; row < board.GetLength(0); row++)
            {
                for (int col = 0; col < board.GetLength(1); col++)
                {
                    if (board[row, col] == Constants.PlayerType.None)
                    {
                        board[row, col] = Constants.PlayerType.PlayerA; // 플레이어의 턴
